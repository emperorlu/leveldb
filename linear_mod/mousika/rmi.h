 ...
 public:
  ...
  inline void prepare(const std::vector<double>& keys,
    ...
    for (unsigned model_i = 0; model_i < models.size(); ++model_i) {
      ...
      if (model.is_lr) {
        ...
        model.lr.prepare(lr_data_in[model_i].first, lr_data_in[model_i].second,
                         lr_index_pred_max, lr_index_pred_min);
  ...
  inline double predict(const double key, unsigned model_i) {
    for (auto& model : models) {
      if (model.range_end < key) continue;
...
class LRStage {
 public:
  ...
  LRStage(const std::string& stages) {
    models.push_back(LinearRegression::deserialize_hardcore(stages));
  }
...
class RMINew {
 public:
  ...
  void finish_insert(bool train_first_layer = true) {
    ...
    for (unsigned model_i = 0; model_i < first_stage->get_model_n();
         ++model_i) {
      ...
      for (int i = 0; i < all_keys.size(); ++i) {
        ...
        unsigned next_stage_model_i = pick_next_stage_model(index_pred);
#ifndef AUF
        second_stage->assign_data(all_keys[i], all_indexes[i],
                                  next_stage_model_i);
#endif
        // real code here
        if (i >= uni_keys.size()) {
  ...
  */
  ...
  void finish_train() {
    ...
    for (unsigned model_i = 0; model_i < second_stage->get_model_n();
         ++model_i) {
